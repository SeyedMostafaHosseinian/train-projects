...
export function shareOperatorFun() {
  ...
  //unshared observable
  const example = source.pipe(tap(() => console.log("*** an new observable is created ***")));
  ...
  const subscribeThree = sharedExample.subscribe((val) =>
    console.log("subscriber 3", ` value : ${val}`)
  );
  const subscribeFour = sharedExample.subscribe((val) =>
    console.log("subscriber 4", ` value : ${val}`)
  );

  // *** an new observable is created ***
  // subscriber 1 value : 0
  // *** an new observable is created ***
  // subscriber 2  value : 0
  // *** an new observable is created ***
  // subscriber 3  value : 0
  // subscriber 4  value : 0
}
